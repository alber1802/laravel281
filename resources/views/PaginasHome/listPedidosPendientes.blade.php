...
<head>
	...
	<title>Lista de pedidos pendientes</title>
			...
			<nav class="full-box navbar-info">
             ...
             <div class="container-fluid">
            	<div class="container-fluid form-neon">
                    <div class="container-fluid">
					<legend> ASIGNACION DE ENTREGA DEL PEDIDO  </legend>
...
<form name="registroEntrega" class="form-neon" action="{{ route('registroEntregas')}}" method="POST" enctype="multipart/form-data">
@csrf
...
<div class="container-fluid">
    ...
    <thead>
        <tr class="text-center">
            ...
            <th>ASIGNAR <input type="checkbox" id="selectAll" /></th>
        ...
        @if(!empty($datos) && count($datos) > 0)
            @foreach($datos as $item)
                <tr class="text-center">
                    ...
                    <td>
                    <input type="checkbox" class="item-checkbox" name="id_pedido" id='id_pedido' value="{{ $item->id_pedido }}" />
                    </td>
                </tr>
            @endforeach
        @else
            <tr>
                <td colspan="11" class="text-center">No hay pedidos disponibles</td>
            </tr>
        @endif
    </tbody>
</table>
...
<fieldset>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-12 col-md-6">
                <div class="form-group">
                    <label for="prestamo_estado" class="bmd-label-floating">Lista de Repartidores</label>
                    <select style="text-align-last: center;" class="form-control" id="id_repartidor" name="id_repartidor">
                        <option value="0">Seleccione un repartidor</option>
                        @if(!empty($repartidores) && count($repartidores) > 0)
                            @foreach($repartidores as $repartidor)
                                <option value="{{ $repartidor->id_repartidor }}">{{ $repartidor->nombre }} {{ $repartidor->paterno }} {{ $repartidor->materno }}</option>
                            @endforeach
                        @else
                            <option value="1">No hay repartidores disponibles</option>
                        @endif
                    </select>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="costo_entrega" id="costo_entrega">
                </div>
            </div>
            <div class="col-12 col-md-4">
                <div class="form-group">
                    <label for="estado_entrega" class="bmd-label-floating">Estado de Entrega</label>
                    <input type="text" class="form-control" name="estado_entrega" id="estado_entrega">
                </div>
            </div>
            <div class="col-12 col-md-4">
                <div class="form-group">
                    <label for="tipo_entrega" class="bmd-label-floating">Tipo de Entrega</label>
                    <input type="text" class="form-control" name="tipo_entrega" id="tipo_entrega">
                </div>
            </div>
        </div>
...
<div class="row">
    <div class="col-12 col-md-10">
        <div class="form-group">
            ...
            <div class="input-group">
                <input type="text" value="{{$direccionC}}" class="form-control" name="direccion_entrega" id="direccion_entrega" placeholder="Ingresa una dirección">
	...
	<input type="hidden" id="direccionOculta"> <!-- Campo oculto para la dirección -->
...
<script>
    ...
    function iniciarMapa() {
        var coordenadas = { lat: -16.48893, lng: -68.14172 };
