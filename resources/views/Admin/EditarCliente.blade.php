	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-account zmdi-hc-fw"></i> Editar cliente <small>COMMCRAFT</small></h1>
			</div>
			<p class="lead">Editar al usuario cliente permite actualizar su información personal, mejorando su experiencia y acceso a productos.</p>
		</div>
		<div class="container-fluid">
			<div class="row">
				<div class="col-xs-12">
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
                                        <form action="{{ route('administracion.clientes.update', $cliente->id_cliente) }}" method="POST">
                                            @csrf
                                            @method('PUT')

									    	<div class="form-group label-floating">
											  <label for="name" class="control-label">Nombre</label>
                                              <input type="text" class="form-control" id="name" name="name" value="{{ $cliente->user->name }}" required>
											</div>
											<div class="form-group label-floating">
											  <label for="paterno" class="control-label">Apellido Paterno </label>
											  <input type="text" class="form-control" id="paterno" name="paterno" value="{{ $cliente->user->paterno }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="paterno" class="control-label">Apellido Materno </label>
                                              <input type="text" class="form-control" id="materno" name="materno" value="{{ $cliente->user->materno }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="nombre" class="control-label">Alias </label>
                                              <input type="text" class="form-control" id="nombre" name="nombre" value="{{ $cliente->user->nombre }}" required>
											</div>
                                            <div class="form-group label-floating">
                                                <label for="sexo">Sexo</label>
                                                <select class="form-control" id="sexo" name="sexo" required>
                                                    <option value="M" {{ $cliente->user->sexo == 'M' ? 'selected' : '' }}>Masculino</option>
                                                    <option value="F" {{ $cliente->user->sexo == 'F' ? 'selected' : '' }}>Femenino</option>
                                                </select>
                                            </div>
                                            <div class="form-group label-floating">
											  <label for="telefono" class="control-label">Telefono </label>
                                              <input type="text" class="form-control" id="telefono" name="telefono" value="{{ $cliente->user->telefono }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="direccion" class="control-label">Direccion </label>
                                              <input type="text" class="form-control" id="direccion" name="direccion" value="{{ $cliente->user->direccion }}" required>
											</div>
											<div class="form-group label-floating">
											...
											<div class="form-group label-floating">
                                              ...
                                              <input type="text" class="form-control" id="preferencia" name="preferencia" value="{{ $cliente->preferencia }}" required>
											</div>
										    ...
										    <p class="text-center">
										    	<button type="submit" class="btn btn-info btn-raised btn-sm"><i class="zmdi zmdi-floppy"></i> Actualizar Cliente</button>
										    </p>
									    </form>
									</div>
								</div>
