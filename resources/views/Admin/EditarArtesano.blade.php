	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-account zmdi-hc-fw"></i> Editar Artesano <small>COMMCRAFT</small></h1>
			</div>
			<p class="lead">Editar al usuario artesano actualiza su información y perfil de productos, asegurando precisión y mejor conexión con los clientes.</p>
		</div>
		<div class="container-fluid">
			<div class="row">
				<div class="col-xs-12">
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
                                        <form action="{{ route('administracion.artesanos.update', $artesano->id_artesano) }}" method="POST">
                                            @csrf
                                            @method('PUT')

									    	<div class="form-group label-floating">
											  <label for="name" class="control-label">Nombre</label>
                                              <input type="text" class="form-control" id="name" name="name" value="{{ $artesano->user->name }}" required>
											</div>
											<div class="form-group label-floating">
											  <label for="paterno" class="control-label">Apellido Paterno </label>
											  <input type="text" class="form-control" id="paterno" name="paterno" value="{{ $artesano->user->paterno }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="materno" class="control-label">Apellido Materno </label>
                                              <input type="text" class="form-control" id="materno" name="materno" value="{{ $artesano->user->materno }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="nombre" class="control-label">Alias </label>
                                              <input type="text" class="form-control" id="nombre" name="nombre" value="{{ $artesano->user->nombre }}" required>
											</div>
                                            <div class="form-group label-floating">
                                                <label for="sexo">Sexo</label>
                                                <select class="form-control" id="sexo" name="sexo" required>
                                                    <option value="M" {{ $artesano->user->sexo == 'M' ? 'selected' : '' }}>Masculino</option>
                                                    <option value="F" {{ $artesano->user->sexo == 'F' ? 'selected' : '' }}>Femenino</option>
                                                </select>
                                            </div>
                                            <div class="form-group label-floating">
											  <label for="telefono" class="control-label">Telefono </label>
                                              <input type="text" class="form-control" id="telefono" name="telefono" value="{{ $artesano->user->telefono }}" required>
											</div>
                                            <div class="form-group label-floating">
											  <label for="direccion" class="control-label">Direccion </label>
                                              <input type="text" class="form-control" id="direccion" name="direccion" value="{{ $artesano->user->direccion }}" required>
											</div>
											<div class="form-group label-floating">
											  <label for="email" class="control-label">Correo Electrónico</label>
                                              <input type="email" class="form-control" id="email" name="email" value="{{ $artesano->user->email }}" required>
											</div>
											<div class="form-group label-floating">
											  <label for="descripcionA" class="control-label">Descripcion</label>
                                              <input type="text" class="form-control" id="descripcionA" name="descripcionA" value="{{ $artesano->descripcionA }}" required>
											</div>
